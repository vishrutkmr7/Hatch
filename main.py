...
parser.add_option(
    "--usernamesel", dest="usernamesel", help="Choose the username selector"
)
parser.add_option("--passsel", dest="passsel", help="Choose the password selector")
parser.add_option(
    "--loginsel", dest="loginsel", help="Choose the login button selector"
)
parser.add_option(
    "--passlist", dest="passlist", help="Enter the password list directory"
)
parser.add_option("--website", dest="website", help="choose a website")
(options, args) = parser.parse_args()


CHROME_DVR_DIR = "/opt/homebrew/bin/chromedriver.exe"
...
def wizard():
    ...
    try:
        ...
        print(f"{color.RED}[!]{color.CWHITE}User used Ctrl-c to exit")
        exit()
    username_selector = raw_input(
        color.GREEN + "[~] " + color.CWHITE + "Enter the username selector: "
    )
    password_selector = raw_input(
        color.GREEN + "[~] " + color.CWHITE + "Enter the password selector: "
    )
    login_btn_selector = raw_input(
...
banner = (
 ...
 |_|  |_|\__,_|\__\___|_| |_|
  ...
  {8}[{9}-{10}]-->{11} brute-force tool                      """.format(
        color.RED,
        color.CWHITE,
        color.RED,
        color.GREEN,
...
if (
    ...
    and options.usernamesel is None
    and options.passsel is None
    and options.loginsel is None
    and options.passlist is None
    and options.website is None
):
    wizard()
